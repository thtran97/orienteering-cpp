...
model::Solution CPSATOPTWSolver::solve(const model::Problem& problem,
    ...
    // ------------------------------------------------------------------
    // Valid inequalities: pairwise time-window ordering / incompatibility.
    //
    // For each pair (i,j), use the reachable earliest departure from source
    // via i (resp. j) to determine whether j can follow i, i can follow j,
    // neither, or both:
    //  - Neither order fits:  skip[i] OR skip[j]  (hard clique cut)
    //  - Only one order fits: force that ordering on start[] when both
    //    are visited (reified via OnlyEnforceIf).
    // These tighten the model beyond what per-arc feasibility filtering
    // captures, because they also apply to *non-adjacent* visited pairs
    // that the circuit's own propagator does not directly reason about.
    // ------------------------------------------------------------------
    const double src_open = problem.get_time_window(source).opening;
    std::vector<double> real_dep(n, 0.0);
    for (int i = 0; i < n; ++i) {
        if (i == source || i == sink) continue;
        const double arr = src_open + problem.get_distance(source, i);
        const double dep = std::max(arr, problem.get_time_window(i).opening)
                           + problem.get_service_time(i);
        real_dep[i] = dep;
    }

    int n_incompat = 0, n_order = 0;
    std::vector<std::vector<bool>> pair_incompat(n, std::vector<bool>(n, false));
    for (int i = 0; i < n; ++i) {
        if (i == source || i == sink) continue;
        const double tw_i_open  = problem.get_time_window(i).opening;
        const double tw_i_close = problem.get_time_window(i).closing;
        ...
        for (int j = i + 1; j < n; ++j) {
            if (j == source || j == sink) continue;
            const double tw_j_open  = problem.get_time_window(j).opening;
            const double tw_j_close = problem.get_time_window(j).closing;
            const double svc_j      = problem.get_service_time(j);

            bool j_after_i = false;
            {
                const double arr_j = real_dep[i] + problem.get_distance(i, j);
                if (arr_j <= tw_j_close + 1e-9) {
                    const double dep_j = std::max(arr_j, tw_j_open) + svc_j;
                    j_after_i = (dep_j + problem.get_distance(j, sink) <= tmax_d + 1e-9);
                }
            }
            bool i_after_j = false;
            {
                const double arr_i = real_dep[j] + problem.get_distance(j, i);
                if (arr_i <= tw_i_close + 1e-9) {
                    const double dep_i = std::max(arr_i, tw_i_open) + svc_i;
                    i_after_j = (dep_i + problem.get_distance(i, sink) <= tmax_d + 1e-9);
                }
            }

            if (!j_after_i && !i_after_j) {
                cp_model.AddBoolOr({skip[i], skip[j]});
                pair_incompat[i][j] = pair_incompat[j][i] = true;
                ++n_incompat;
            } else if (!j_after_i) {
                cp_model.AddLessOrEqual(LinearExpr(start[j]),
                                        LinearExpr(start[i]) - 1)
                         .OnlyEnforceIf({skip[i].Not(), skip[j].Not()});
                ++n_order;
            } else if (!i_after_j) {
                cp_model.AddLessOrEqual(LinearExpr(start[i]),
                                        LinearExpr(start[j]) - 1)
                         .OnlyEnforceIf({skip[i].Not(), skip[j].Not()});
                ++n_order;
            }
        }
    ...
    // Triple-infeasibility cuts: for each triple (i,j,k) where no pair is
    // already pairwise incompatible, check if all 6 orderings of
    // source→a→b→c→sink are infeasible. If so, at most 2 of {i,j,k} can be
    // visited.
    int n_triple = 0;
    for (int i = 0; i < n; ++i) {
        if (i == source || i == sink) continue;
        for (int j = i + 1; j < n; ++j) {
            if (j == source || j == sink) continue;
            if (pair_incompat[i][j]) continue;
            for (int k = j + 1; k < n; ++k) {
                if (k == source || k == sink) continue;
                if (pair_incompat[i][k] || pair_incompat[j][k]) continue;

                const int nodes[3] = {i, j, k};
                static const int perm[6][3] = {
                    {0,1,2},{0,2,1},{1,0,2},{1,2,0},{2,0,1},{2,1,0}};
                bool any_feasible = false;
                for (const auto& p : perm) {
                    const int a = nodes[p[0]], b = nodes[p[1]], c = nodes[p[2]];
                    double t = src_open + problem.get_distance(source, a);
                    if (t > problem.get_time_window(a).closing + 1e-9) continue;
                    t = std::max(t, problem.get_time_window(a).opening)
                        + problem.get_service_time(a);
                    t += problem.get_distance(a, b);
                    if (t > problem.get_time_window(b).closing + 1e-9) continue;
                    t = std::max(t, problem.get_time_window(b).opening)
                        + problem.get_service_time(b);
                    t += problem.get_distance(b, c);
                    if (t > problem.get_time_window(c).closing + 1e-9) continue;
                    t = std::max(t, problem.get_time_window(c).opening)
                        + problem.get_service_time(c);
                    t += problem.get_distance(c, sink);
                    if (t <= tmax_d + 1e-9) { any_feasible = true; break; }
                }
                if (!any_feasible) {
                    cp_model.AddBoolOr({skip[i], skip[j], skip[k]});
                    ++n_triple;
                }
            }
        }
    ...
    // ------------------------------------------------------------------
    // NoOverlap on service intervals.
    //
    // The single vehicle can serve at most one customer at a time.
    // Modelling each active (not-skipped) customer as an OptionalInterval
    // [start[i], start[i]+svc[i]) gives CP-SAT's scheduling propagators
    // (timetable, edge-finding, precedence/core reasoning) an independent
    // reasoning path to derive objective bounds, complementing the
    // circuit constraint's own propagation.
    // ------------------------------------------------------------------
    {
        std::vector<IntervalVar> svc_ivals;
        svc_ivals.reserve(n);
        for (int i = 0; i < n; ++i) {
            if (i == source || i == sink) continue;
            const int64_t svc = scaled_svc(i);
            if (svc <= 0) continue;
            svc_ivals.push_back(
                cp_model.NewOptionalFixedSizeIntervalVar(
                    start[i], svc, skip[i].Not()));
        }
        if (!svc_ivals.empty())
            cp_model.AddNoOverlap(svc_ivals);
    }

    if (config.verbose)
        std::cout << "[CPSAT_OPTW] Valid ineqs: "
                  << n_incompat << " incompatibility, "
                  << n_order   << " ordering, "
                  << n_triple  << " triple\n";
