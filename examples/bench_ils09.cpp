...
int main(int argc, char** argv) {
    auto opts = bench::parse_cli(argc, argv, "ils09");

    // --- ILS09 config (adjustable) ---
    metaheuristic::ILS09Solver solver;
    metaheuristic::ILS09SolverConfig cfg;
    cfg.seed              = opts.seed;
    ...
    auto csv_map = bench::open_csv_files(opts.output_root, "ils09", opts.variants, opts.overwrite);
    int ok = 0, err = 0;
    for (const auto& spec : instances) {
        ...
        for (int run = 1; run <= opts.runs; ++run) {
            auto r = bench::run_and_record(solver, cfg, *problem, spec, "ils09", run);
