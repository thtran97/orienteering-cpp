...
int main(int argc, char** argv) {
    ...
    routing::RoutingOPTWSolver solver;
    routing::RoutingOPTWSolverConfig cfg;
    cfg.seed         = opts.seed;
    cfg.max_cpu_time = opts.timeout;
    cfg.verbose      = opts.verbose;
    // local_search_metaheuristic defaults to TABU_SEARCH (4).
    ...
    auto csv_map = bench::open_csv_files(opts.output_root, "routing_optw", opts.variants, opts.overwrite);
    int ok = 0, err = 0;
    for (const auto& spec : instances) {
        ...
        for (int run = 1; run <= opts.runs; ++run) {
            auto r = bench::run_and_record(solver, cfg, *problem, spec, "routing_optw", run);
