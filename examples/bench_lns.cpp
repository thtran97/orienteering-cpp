...
int main(int argc, char** argv) {
    auto opts = bench::parse_cli(argc, argv, "lns");

    // --- LNS config (adjustable) ---
    metaheuristic::LNSSolver solver;
    metaheuristic::LNSSolverConfig cfg;
    cfg.seed           = opts.seed;
    ...
    auto csv_map = bench::open_csv_files(opts.output_root, "lns", opts.variants, opts.overwrite);
    int ok = 0, err = 0;
    for (const auto& spec : instances) {
        ...
        for (int run = 1; run <= opts.runs; ++run) {
            auto r = bench::run_and_record(solver, cfg, *problem, spec, "lns", run);
