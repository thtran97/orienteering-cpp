...
int main(int argc, char** argv) {
    auto opts = bench::parse_cli(argc, argv, "mcts");

    // --- MCTS config (adjustable) ---
    policy_learning::MCTSSolver solver;
    policy_learning::MCTSSolverConfig cfg;
    cfg.seed           = opts.seed;
    ...
    auto csv_map = bench::open_csv_files(opts.output_root, "mcts", opts.variants, opts.overwrite);
    int ok = 0, err = 0;
