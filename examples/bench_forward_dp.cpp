...
int main(int argc, char** argv) {
    auto opts = bench::parse_cli(argc, argv, "forward_dp");

    // --- Forward DP config (adjustable) ---
    dp::ForwardDPSolver solver;
    ...
    auto csv_map = bench::open_csv_files(opts.output_root, "forward_dp", opts.variants, opts.overwrite);
    int ok = 0, err = 0;
    for (const auto& spec : instances) {
        ...
        for (int run = 1; run <= opts.runs; ++run) {
            auto r = bench::run_and_record(solver, cfg, *problem, spec, "forward_dp", run);
