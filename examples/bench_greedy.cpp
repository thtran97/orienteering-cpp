...
int main(int argc, char** argv) {
    auto opts = bench::parse_cli(argc, argv, "greedy");

    // --- Greedy config (adjustable) ---
    constructive::GreedySolver solver;
    constructive::GreedySolverConfig cfg;
    cfg.seed           = opts.seed;
    cfg.max_cpu_time   = opts.timeout;
    cfg.max_iterations = opts.iterations;
    cfg.verbose        = opts.verbose;
    cfg.move_evaluator = MoveEvaluatorType::REWARD;
    ...
    auto csv_map = bench::open_csv_files(opts.output_root, "greedy", opts.variants, opts.overwrite);
    int ok = 0, err = 0;
    for (const auto& spec : instances) {
        ...
        for (int run = 1; run <= opts.runs; ++run) {
            auto r = bench::run_and_record(solver, cfg, *problem, spec, "greedy", run);
